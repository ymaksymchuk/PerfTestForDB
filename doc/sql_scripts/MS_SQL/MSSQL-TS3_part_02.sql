    ...
    BEGIN
        ...
        FROM
            dbo.CMS_CASE_FILE CF (NOLOCK)
                ...
                LEFT JOIN dbo.CMS_CASE_TYPE CT WITH (NOLOCK) ON CT.CASE_TYPE_ID=C.CASE_TYPE_ID
                LEFT JOIN dbo.CMS_CASE_STATUS CS WITH (NOLOCK) ON C.CASE_ID=CS.CASE_ID AND CS.CASE_STAT_CUR_STAT=1
                LEFT JOIN dbo.CMS_STATUS_TYPE ST WITH (NOLOCK) ON CS.STATUS_ID=ST.STATUS_ID
                LEFT OUTER JOIN dbo.CMS_DELAY_CODE DC WITH (NOLOCK) ON DC.DELAY_CODE_ID=CS.DELAY_CODE_ID
                LEFT JOIN dbo.CMS_CLIENT_HISTORY CH  WITH (NOLOCK) ON CH.CASE_FILE_ID = CF.CASE_FILE_ID AND CH.CLIENT_ROLE_ID=1 AND CH.CLIENT_HIST_CUR_CLIENT=1
                LEFT JOIN dbo.CMS_CLIENT CLI  WITH (NOLOCK) ON CLI.CLIENT_ID = CH.CLIENT_ID
